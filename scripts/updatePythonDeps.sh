...
#pipenv shell
DEBIAN_FRONTEND=noninteractive sudo apt-get install -y --no-install-recommends pipenv python3.14
python3.14 -m pip install --no-cache-dir --upgrade pip --break-system-packages
python3.14 -m pip install --no-cache-dir --upgrade --user pipenv certifi wheel setuptools packaging --break-system-packages
...
do
	...
	pipenv --python 3.14 lock -v && pipenv --python 3.14 requirements --exclude-markers > requirements.txt
